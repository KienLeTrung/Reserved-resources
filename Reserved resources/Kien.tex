...
\begin{abstract}
Ultra-Reliable Low-Latency Communication is a new service in 5G with strict requirements of reliability and latency. To meet these requirements in the uplink, grant-free uplink transmission has been specified, allowing the UE to transmit data in a random-access fashion without first transmitting a scheduling request and then waiting for a scheduling grant from the gNB. To further increase the reliability, these grant-free UL transmissions can be repeated without waiting for HARQ feedback from the gNB. However, these repetitions have to happen within a certain interval avoid a confusion in HARQ IDs of different HARQ processes. When a UE starts transmitting late in the interval it therefore can not exploit all the possible repetitions and thus reliability and latency is decreased. In this paper, a scheme based on reserved resources is proposed to ensure the number of repetitions in a specific period. The size of each reserved resource is optimized depending on its position so as to reduce resource consumption. The scheme evaluated by theoretical analysis and numerical results shows its benefits to system performance.
\end{abstract}
...
We consider one UE of interest having a transmission in the reserved resource at t21 and the rest of $N-1$ UEs [...]. %TODO Kien: sentence incomplete!!!
